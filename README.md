...
| **worker** | `openrouter/z-ai/glm-5.3` | `read`, `write`, `edit`, `bash`, `web_search`, `fetch_content`, `get_search_content`, `source_check` + spawning | General implementer; may spawn `scout` and `researcher` |
...
| `tools` | string | Strict tool allowlist. Built-ins: `read`, `write`, `edit`, `bash`, `grep`, `find`, `ls`. Extension-backed: `web_search`, `fetch_content`, `safe_bash`, `video_extract`, `youtube_search`, `google_image_search`. `pi-web-access` also exposes `source_check` and `get_search_content`. Only the extensions backing the listed tools are loaded into the child |
